...
void MainWindow::on_loadButton_clicked()
{
    QString fileName;
    fileName =
        QFileDialog::getOpenFileName(this, "Выберите файл", QDir::homePath(), "Images (*.png *.bmp *.jpg *.jpeg)");
    ...
    if (!fileName.isEmpty())
    {
        ...
        if (pixmap.load(fileName))
        {
            ...
            double heightRatio = static_cast<double>(pixmap.height()) / ui->pictureLabel->height();
            double widthRatio = static_cast<double>(pixmap.width()) / ui->pictureLabel->width();
            ...
            image.load(fileName);
            palette.clear();
            QList<QLabel*> labelList{ui->color1,     ui->color2,     ui->color3,     ui->color4,
                                     ui->color5,     ui->color6,     ui->textColor1, ui->textColor2,
                                     ui->textColor3, ui->textColor4, ui->textColor5, ui->textColor6};
...
{
    return sqrt(0.299 * pow(color1.red() - color2.red(), 2) + 0.114 * pow(color1.blue() - color2.blue(), 2) +
...
void MainWindow::showPalette(QImage image)
{
    ...
    for (int x = 0; x < image.width(); ++x)
    {
        for (int y = 0; y < image.height(); ++y)
        {
            ...
            if (pixel.alpha() == 0)
            {
                continue; // полностью прозрачные пиксели не участвуют в подсчёте
            }
            QString colorName = pixel.name();
    ...
    QList<QLabel*> textLabelList{ui->textColor1, ui->textColor2, ui->textColor3,
                                 ui->textColor4, ui->textColor5, ui->textColor6};
    QList values = colorMap.values();
    if (values.isEmpty())
    {
        return; // изображение полностью прозрачное
    }
...
void MainWindow::on_exportButton_clicked()
{
    ...
    {
        QString fileName =
            QFileDialog::getSaveFileName(this, "Сохраните файл", QDir::homePath() + "/palette.png", "Images (*.png)");
        if (fileName.isEmpty())
            return;
        int kBlockWidth = 50;
        int kBlockHeight = 25;
