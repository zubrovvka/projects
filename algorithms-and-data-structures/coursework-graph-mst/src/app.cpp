...
int App::run()
{
    while (run_proc)
    {
        ...
        if (matrix == NULL)
        {
            // Файл не прочитан — работать не с чем
            return 1;
        }
        showMenu();
#ifdef _WIN32
...
void App::readMatrixFromFile()
{
    ...
    while (!err && getline(f_in, s))
    {
        ...
        if (row == msize)
        {
            ...
            {
                // Пустая строка в конце файла — просто пропускаем
                break;
            }
            err = true;
        ...
        while (!err && i < len)
        {
            ...
            else if (s[i] == ' ' || s[i] == '\t' || s[i] == '\r')
            {
                if (token != "")
                {
                    ...
                    if (val >= EDGE_VAL_MIN && val <= EDGE_VAL_MAX)
                    {
                        ...
                        {
                            // err = true;
                            // cout << "Value row: " << row << " col: " << col << " val: " << val << " isn't
                            // symmetrical" << endl;
                        }
                        token = "";
                        col++;
            ...
            if (i == len && token != "")
            {
                ...
                if (val >= EDGE_VAL_MIN && val <= EDGE_VAL_MAX)
                {
                    ...
                    {
                        // err = true;
                        // cout << "Value row: " << row << " col: " << col << " val: " << val << " isn't symmetrical" <<
                        // endl;
                    }
                    token = "";
                    col++;
    ...
    if (!err)
    {
        cout << "File " << filepath << " was read successfully" << endl;
    }
    else
    {
        initArrays();
...
// Освобождение массива посещений
void App::freeVisited()
{
    delete[] visited;
    visited = NULL;
}
...
void App::bfs(const int v)
{
    ...
    while (!q.empty())
    {
        ...
        v_ind = q.front();
        q.pop();
        // всех связных первого уровня непосещённых посещаем и в очередь
        for (int j = 0; j < msize; j++)
        {
