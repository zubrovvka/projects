// Реализация шаблонного класса List (подключается из list.h)

template <class T> void List<T>::resize(unsigned new_size)
{
    ...
    else
    {
        ...
        for (unsigned i = 0; i < list_size - new_size; i++)
        {
            m = head->next;
            delete head;
            head = m;
...
template <class T> T& List<T>::operator[](int index)
{
    ...
    if (index < 0 || static_cast<unsigned>(index) >= list_size)
        exit(1);
    Node* m = head;
...
template <class T> List<T>& List<T>::operator=(List<T> const& obj)
{
    if (this != &obj)
    {
...
template <class T> int List<T>::find(const T& val)
{
    Node* m = head;
    int i = 0;
    while (m)
...
template <class T> bool List<T>::contains(const T& val)
{
    if (find(val) == -1)
        return false;
    return true;
...
template <class T> bool List<T>::insert(unsigned int pos, const T& val)
{
...
template <class T> T List<T>::pop(int index)
{
    if (index < 0)
        index = list_size + index;
    if (index < 0)
...
template <class T> std::ostream& operator<<(std::ostream& buff, List<T>& list)
{
    buff << '[';
    int size = list.size();
